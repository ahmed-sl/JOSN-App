...
class MainActivity : AppCompatActivity() {
    var curencyDetails: Datum? = null

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_main)

        val userinp = findViewById<View>(R.id.InputNum) as EditText
        val sub = findViewById<View>(R.id.Submet) as Button
        val spinner = findViewById<View>(R.id.spr) as Spinner

        val lis = arrayListOf("sar", "usd", "inr", "btc", "cny", "jpy")
        ...
        sub.setOnClickListener {
            ...
            getCurrency(onResult = {
                ...
                when (selec) {
                    0 -> disp(calc(curencyDetails?.eur?.sar?.toDouble(), currency));
                    1 -> disp(calc(curencyDetails?.eur?.usd?.toDouble(), currency));
                    2 -> disp(calc(curencyDetails?.eur?.aud?.toDouble(), currency));
                    3 -> disp(calc(curencyDetails?.eur?.inr?.toDouble(), currency));
    ...
    private fun disp(calc: Double) {
        ...
        val responseText = findViewById<View>(R.id.Result) as TextView
